...
# We are focusing on Redundancy Analysis (RDA).
  ...
  # pca() and cca() can typically just replace the rda() in any of these functions and models.
        #**easy to change if not using RDA
...
pl <- ordiellipse(mod3, dune.env$Management, kind="se", conf=0.95, lwd=2, draw = "polygon", 
                  col="skyblue", border = "blue") ##creates the circles on plot grouping them
summary(pl)
...
##here use is not signifigant as seen in the plot since they are all on top of one another
summary(pl)
...
Site_Scores <- mod3$CCA$u #u is site
...
#We'll just focus on the site scores and plot Axis 2 against Axis 3:
    ##instead of first and second RDA we can look at third and fourth
